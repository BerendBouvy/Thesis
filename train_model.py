...
def train_model():
    ...
    model = VAE(input_dim=50, latent_dim=latent_dim, density=1, beta=1e-2).to(device)
    optimizer = torch.optim.AdamW(model.parameters(), lr=learning_rate, weight_decay=weight_decay)

    writer = SummaryWriter(f'runs/6/vae_{datetime.now().strftime("%Y%m%d-%H%M%S")}')
...
if __name__ == "__main__":
    paths = ['data/sim_10000_50_25_1_0.25/set_2/data.csv']
    learning_rates = [1e-3]
    weight_decays = [1e-2]
    latent_dims = [16]
    train_model()
