...
class VAE(nn.Module):
    ...
    def __init__(self, input_dim, latent_dim, density: int=1, beta: float=1.0):
        ...
        self.beta = beta
        layers = []
        layers.append(nn.Linear(input_dim, 10))  # Encoder output layer
        layers.append(nn.SiLU())
        for _ in range(3):
            layers.append(nn.Linear(10, 10))
            layers.append(nn.SiLU())
            
        layers.append(nn.Linear(10, 2 * latent_dim))  # Output layer for mu and logvar
        layers.append(nn.SiLU())
        ...
        for _ in range(3):
            decoder.append(nn.Linear(10, 10))
            decoder.append(nn.SiLU())
