...
from VAE6 import VAE, VAEOutput
from dataLoader import data_loader
from train6 import train
from test_model6 import test
import os
import copy
...
def train_model(source: str, paths: list[str], learning_rate: float, weight_decay: float, num_epochs: int, latent_dim: int, density: int, beta, target: bool, batch_size: int, writer: SummaryWriter = None, verbose: bool = True):
    ...
    for path in paths:
        ...
        writer = SummaryWriter(f'runs61/{source}vae_{path}_{datetime.now().strftime("%Y%m%d-%H%M%S")}')
        # Load the dataset
        ...
        for epoch in range(num_epochs):
            ...
            if test_loss < best_loss:
                best_loss = test_loss
                # Save a copy of the best state dict (call state_dict())
...
if __name__ == "__main__":
    source = "data13/1_sim_2000_200_20_0.45_0.45_0.7_1"
    paths = os.listdir(source)
    paths.remove('formatted_scores.csv')
    learning_rates = 1e-3
    weight_decays = 1e-5
    num_epochs = 200
    latent_dims = 20
    density = 1
    beta = np.linspace(0, 5, num_epochs) # 1e-3
    batch_size = 512
    target = True
    verbose = True
    output = train_model(source, paths, learning_rates, weight_decays, num_epochs, latent_dims, density=density, beta=beta, batch_size=batch_size, target=target, writer=None, verbose=verbose)
